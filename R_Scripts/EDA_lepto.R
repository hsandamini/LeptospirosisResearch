...
# Build ONE combined dataset: every year's data joined to the shapefile at once
map_all_years <- lka_adm2 %>%
  left_join(yearly_district, by = c("district_clean" = "district")) %>%
  mutate(annual_cases = replace_na(annual_cases, 0),
         case_bin = case_when(
           annual_cases <= 50  ~ "0-50",
           annual_cases <= 150 ~ "51-150",
...
p_all <- ggplot(map_all_years) +
  geom_sf(aes(fill = case_bin), colour = "white", linewidth = 0.15) +
  facet_wrap(~ year, ncol = 5) +
  scale_fill_manual(values = yearly_bin_colours, name = "Cases", drop = FALSE) +
  labs(title = "Leptospirosis Cases by District, 2007–2025") +
  theme_minimal(base_size = 8) +
  theme(axis.text = element_blank(), axis.ticks = element_blank(),
        panel.grid = element_blank(),
...
ggsave("choropleth_all_years_combined.png", p_all, width = 20, height = 16, dpi = 300, limitsize = FALSE)
