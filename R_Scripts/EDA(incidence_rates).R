...
view(district_incidence_stats, n = Inf)
...
districts <- sort(unique(lepto_incidence$district))   # <-- fixed
...
# Step 7 — District x month heatmap panels (incidence categories), ordered by mean incidence (descending)

summary(lepto_incidence$incidence_per_100k)   # check range before setting bins

# Order districts by mean incidence, descending (highest at top)
district_order_desc <- district_incidence_stats %>%
  arrange(desc(mean_incidence)) %>%
  pull(district)
...
# Build ONE combined dataset: every year's incidence joined to the shapefile at once
...
# One combined panel, faceted by year
p_all <- ggplot(map_all_years) +
  geom_sf(aes(fill = inc_bin), colour = "white", linewidth = 0.15) +
  facet_wrap(~ year, ncol = 5) +
  scale_fill_manual(values = yearly_bin_colours, name = "Incidence\nper 100k", drop = FALSE) +
  labs(title = "Leptospirosis Incidence by District, 2007–2025") +
  theme_minimal(base_size = 8) +
...
ggsave("incidence_choropleth_all_years_combined.png", p_all,
       width = 20, height = 16, dpi = 300, limitsize = FALSE)
